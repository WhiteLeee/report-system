...
- PostgreSQL 存储：`report / report_store / report_image / report_issue / report_review_log`
...
├── backend/
│   ├── auth/                         # 请求上下文
│   ├── database/                     # PostgreSQL / schema / migrate
...
如果你刚切过 Node 版本（或看到 `pg` 的 `NODE_MODULE_VERSION` 报错），先执行：
...
- `deploy.sh` 内置 PostgreSQL 备份（`pg_dump`），默认保留天数在配置文件中可调。
...
如果测试前刚切换过 Node 版本，建议先执行 `npm install`，避免原生模块 ABI 不匹配。

该命令会执行 `tests/*.test.ts` 下的全部测试用例（包含集成与纯函数测试）。

## 7. 数据库

PostgreSQL 连接：
- `REPORT_SYSTEM_DB_URL=postgres://postgres:postgres@127.0.0.1:5432/report_system`
- `REPORT_SYSTEM_DB_MIGRATIONS_SCHEMA=public`
- `REPORT_SYSTEM_DB_MIGRATIONS_TABLE=__drizzle_migrations`

迁移说明：
- 当前运行时与部署脚本已统一切换为 PostgreSQL。
- 新环境初始化仅需执行 `npm run db:migrate` 同步 schema。
- 生产发布统一使用 `deploy/prod` 脚本，并使用 `backup-db.sh` / `restore-db.sh` 做 PostgreSQL 备份与回滚。
...
- `db:reset:local`：清空本地 PostgreSQL schema 并重建表结构
- `reinit:local`：重建当前租户配置与本地数据库，适合测试阶段快速回到初始状态
- `REPORT_SYSTEM_DB_MIGRATIONS_SCHEMA/TABLE`：控制迁移元数据表存放位置（默认 `public.__drizzle_migrations`）
...
2. 当前默认单库部署，访问量上升后需评估连接池、索引和读写扩展策略。
...
4. 持续完善 PostgreSQL 运维基线（备份恢复演练、索引治理、慢查询监控）。
